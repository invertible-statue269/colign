...
func (s *Service) Archive(ctx context.Context, changeID, userID int64) (*models.Change, error) {
...
func (s *Service) Unarchive(ctx context.Context, changeID, userID int64) (*models.Change, error) {
	change := new(models.Change)
	err := s.db.NewSelect().Model(change).Where("id = ?", changeID).Scan(ctx)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
